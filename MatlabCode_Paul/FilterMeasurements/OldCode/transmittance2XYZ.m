...
function XYZ = transmittance2XYZ (trans_array, sizey, sizex, ls)

    disp('Combining tranmittance and illuminant into XYZ...')
    
    % Compute Xn, Yn, Zn using the chosen illuminant
    XYZxyz0 = spd2XYZ(ls);
    whiteY = XYZxyz0(2);  % Yn
    
    ls_array = repmat(ls,1,sizey*sizex);
    spd_array = trans_array .* ls_array;
    
    % rescale to [0:1]
    % define the white level here
    XYZ = spd2XYZ(spd_array)/whiteY * 1; % Here k = 1

end

% Cumputes the XYZ tri-stimulus coordinates
function XYZ = spd2XYZ (s);
