...
function [LAB, CovLAB, XYZ, CovXYZ] = f_transmittance2LAB(trans_array_m, trans_array_s, sizey, sizex, ls, trim)
    
    % Set the max input T to 1 and proportionally scales the uncertainty
    ...
    % Reference white -- converts spectrum data into CIEXYZ space
    XYZ0 = f_spd2XYZ(1, [], ls);
    ...
    disp('  calculate XYZ...')
    % Convert spectrum data into CIEXYZ space
    [XYZ, CovXYZ] = f_spd2XYZ(trans_array_m, trans_array_s, ls_array);

    disp('  calculate LAB...')
    % Convert CIEXYX data into CIELab space
