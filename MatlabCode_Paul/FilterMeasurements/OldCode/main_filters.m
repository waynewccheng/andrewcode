...
function main_filters(name_of_sample, ol490)

close all;
% clearvars -except ol490 ludl pr name_of_sample
...
% 
% % Filter informations
% name_of_sample = 'Filter_TestPhantom2'

% Gathers and stores the measurements informations, creates the folders
[p_sample, p_white, p_spectro,...
    p_trans, p_cie, p_st] = init_filters(pr, name_of_sample);
p_sw = {p_sample, p_white};

%% 2: ROI
[xy, xy_white] = findroi_PL3(ol490, ludl, 'w'); % 'g' for green light, 'w' for broadband
ROI = [xy; xy_white];

%% 3: SpectroRadiometer: Measurements
% Number of repeated measurements
n_meas = 10; 

% Call of measurements by spectrometer
[lambda, spectra, spectra_background] = f_meas_spectro_PL3(ol490, pr, ludl, n_meas, ROI);
...
%% 4: SpectroRadiometer: Calculate LAB: T -> XYZ -> LAB
...
[LAB_spectro, CovLAB_spectro, XYZ_spectro, CovXYZ_spectro] = transmittance2LAB_PL2(t_mean_spectro(1:10:401)', t_std_spectro(1:10:401)', 1, 41, ls, 'n');
...
%% 5: Camera: Establishes/get the maximum shutter time, spanning the wavelengths; Set the OL490 max intensities 
numberofshots = 1;
bandwidth = 10;

% Camera Shutter time
shutter_tbl = f_shutter_time(ol490, ludl, ROI, numberofshots, bandwidth, p_st, 1); % 0 just loads the shutter times established previously, set 1 to establish them
...
for j = 1:2 % 1 is filter, 2 is 100% transmittance
    ...
    camera2frame_9MP_small_PL3(p_sw{j}, numberofshots, ol490, int8(intensities(:, 2)), bandwidth, shutter_tbl(:, 2));
...
%% 7: Camera: Calculate transmittance
...
% pixel values
[trans_ms, trans_array_m, trans_array_s, sizey, sizex] = frame2transmittance_white_PL4(p_sample, p_white, numberofshots);
...
%% 8: Camera: Calculate LAB
% Trans -> XYZ -> LAB with saving XYZ and LAB (values + uncertainties)
[LAB_cam, CovLAB_cam, XYZ_cam, CovXYZ_cam] = transmittance2LAB_PL2(trans_ms(:, 2), trans_ms(:, 3), 1, 41, ls, 'n');
[LAB_array, CovLAB_array, XYZ_array, CovXYZ_array] = transmittance2LAB_PL2(trans_array_m, trans_array_s, sizey, sizex, ls, 'n');
